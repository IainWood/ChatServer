...
public class Project4Test {
    ...
    public void testCookieTimeout1() throws InterruptedException{
        SessionCookie.timeoutLength = 1;  //change timeout interval to 1 second

        SessionCookie cookie = new SessionCookie(1234);

        assertEquals(cookie.hasTimedOut(), false); // should print false

        Thread.sleep(1200); // Sleep for 1.2 seconds

        assertEquals(cookie.hasTimedOut(), true); // should print true
    }

    @Test(timeout=1000000)
    public void testCookieTimeout2() throws InterruptedException{
        SessionCookie.timeoutLength = 1;  //change timeout interval to 1 second
        ...
        Thread.sleep(999); // Sleep for 1.2 seconds

        assertEquals(cookie.hasTimedOut(), false); // should print true
    }
    ...
    public void getMessageCheck() {
        ...
        ChatServer chatServer = new ChatServer(users, 2);

        String msg = "Message 0";

        String expected = "SUCCESS\t0002) root: Message: 2\t0003) root: Message: 3\r\n";

        chatServer.postMessage(new String[] { "POST-MESSAGE", "42", msg },  "root");
        msg = "Message: 1";
        chatServer.postMessage(new String[] { "POST-MESSAGE", "42", msg },  "root");
        msg = "Message: 2";
        chatServer.postMessage(new String[] { "POST-MESSAGE", "42", msg },  "root");
        ...
        String actual = chatServer.getMessages(new String[] {"GET-MESSAGES" , "42" , "10"} );
